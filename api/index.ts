...
// DB init flag to avoid re-running on every request
...
// Middleware: ensure DB is initialized before handling requests
...
app.post("/api/payments/generate", async (req, res) => {
    ...
    try {
        ...
        // Get all active students (or those with attendance in this month)
        const rsStudents = await db.execute({
            sql: `SELECT id, name, student_code, class_name FROM students 
                  WHERE status = 'active' 
                     OR id IN (SELECT student_id FROM attendance WHERE date BETWEEN ? AND ?)
        ...
        // Delete existing payments for this month then re-insert
